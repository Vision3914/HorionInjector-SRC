...
namespace HorionInjector
{
	...
	public partial class MainWindow : Window
	{
		...
		private void Inject(string path)
		{
			...
			else
			{
				...
				for (int i = 0; i < process.Modules.Count; i++)
				{
					...
					{
						MessageBox.Show("Already injected you sussy baka!");
						goto IL_2CD;
					}
				}
		...
		public MainWindow()
		{
			string path = Path.ChangeExtension(Assembly.GetExecutingAssembly().Location, "old");
			if (File.Exists(path))
			{
				File.Delete(path);
			}
		...
		private void SetStatus(string status)
		{
			if (status == "done")
			{
				...
				Application.Current.Dispatcher.Invoke(DispatcherPriority.Render, new Action(delegate()
				{
					this.InjectButton.Content = "inject sussy code!!";
				}));
			}
			else
			{
		...
		private void InjectButton_Left(object sender, RoutedEventArgs e)
		{
			...
			if (!this.CheckConnection() && MessageBox.Show("Can I inject this sussy DLL?", null, MessageBoxButton.YesNo) == MessageBoxResult.No)
			{
				this.SetStatus("done");
				return;
			}
			...
			string file = Path.Combine(Path.GetTempPath(), "Horion.dll");
			webClient.DownloadFileCompleted += delegate(object _, AsyncCompletedEventArgs __)
			{
				this.Inject(file);
			};
			webClient.DownloadFileAsync(new Uri("https://horion.download/bin/Horion.dll"), file);
		}
